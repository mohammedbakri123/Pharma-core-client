...
import {
  usePurchaseReturnPayments,
  useAddPurchaseReturnPayment,
} from "@features/finance/common/hooks/usePayments";
import { usePurchaseReturnBalance } from "@features/finance/common/hooks/usePurchaseReturns";
...
interface PurchaseReturnPaymentSectionProps {
  purchaseId: number;
  returnId: number;
  isCompleted: boolean;
}

export default function PurchaseReturnPaymentSection({
  purchaseId,
  returnId,
  isCompleted,
}: PurchaseReturnPaymentSectionProps) {
  const [searchParams] = useSearchParams();
  const page = Number(searchParams.get("page") ?? "1");
  const limit = 10;

  const { data: paymentsData, isLoading } = usePurchaseReturnPayments(returnId);
  const addPaymentMutation = useAddPurchaseReturnPayment(purchaseId, returnId);
  const { data: balance } = usePurchaseReturnBalance(purchaseId, returnId);
  ...
  return (
    <div>
      ...
      <DataTable
        ...
        isLoading={isLoading}
        emptyMessage="لا توجد مدفوعات لهذا المرتجع"
      />
