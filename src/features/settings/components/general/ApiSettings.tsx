...
export default function ApiSettings() {
  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      toast({
        ...
        title: "فشل الاتصال",
        description:
          "تعذر الوصول إلى خادم واجهة البرمجة (.NET API). يرجى التحقق من تشغيل الخادم.",
      });
    } finally {
      setIsTesting(false);
    }
  ...
  const getStatusBadge = () => {
    ...
    return (
      <Badge className="bg-emerald-500 hover:bg-emerald-600 text-white flex items-center gap-1">
        <Wifi className="w-3 h-3" /> متصل ونشط
      </Badge>
    );
  };
  ...
  const getDatabaseStatusBadge = () => {
    if (isLoading)
      return (
        <span className="text-muted-foreground text-sm">جاري الفحص...</span>
      );
    ...
    <div className="space-y-6 text-right" dir="rtl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-2">
          ...
          >
            <Server className="w-4 h-4 text-primary" /> عنوان خادم التطبيق (Base
            URL)
          </Label>
          <Input
            id="api-base-url"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-2">
          <Label className="flex items-center gap-1.5 justify-end">
            <Clock className="w-4 h-4 text-primary" /> إصدار نظام التشغيل
            (Build)
          </Label>
          <Input
            id="api-version"
            value={
              isLoading
                ...
                : isError
                  ? "غير متاح"
                  : healthResponse?.version || "1.0.0.0"
            }
            readOnly
        ...
        <div className="space-y-2">
          <Label className="flex items-center gap-1.5 justify-end">
            <Database className="w-4 h-4 text-primary" /> قاعدة بيانات
            PostgreSQL
          </Label>
          <div className="h-10 flex items-center justify-between border border-border/40 px-3 bg-background rounded-md">
